...
from .attention import CommBlock
...
class Critic(nn.Module):
    def __init__(self, model_config, observation_space, action_space, custom_config, initialization):
        ...
        self.comm=CommBlock(self.hidden_dim,self.hidden_dim,self.hidden_dim,8)
    ...
    def forward(self, **kwargs):
        ...
        observations=observations[...,:-5-self.num_edges*3]
        # B*A, 2
        edge_idices=observations[...,-5-self.num_edges*3:-5-self.num_edges].long()
        # B*A
        edge_masks=observations[...,-5-self.num_edges:-5].long()
        
        index_offsets=(torch.arange(B,device=edge_idices.device,dtype=edge_idices.dtype)*A).reshape(-1,1).repeat(1,A).reshape(-1,1)
        edge_idices+=index_offsets
        edge_idices=edge_idices.reshape(-1,2)
        edge_masks=edge_masks.reshape(-1)
        edge_idices=edge_idices[edge_masks].transpose(0,1)
        ...
        # B*A,1
        h_comm=self.comm(h, edge_idices)
        values = self.out(h+h_comm)
...
class Actor(nn.Module):
    def __init__(
        ...
        # TODO: maybe we should include location as features
        self.comm=CommBlock(self.hidden_dim,self.hidden_dim,self.hidden_dim,8)
