...
class EpisodeLog:
    ...
    def add_starts(self, locs: torch.Tensor):
        if not self.enabled:
            return
        self.log["start"]=[loc+["E"] for loc in locs.cpu().numpy().tolist()]
        
    def add_actions(self, actions: torch.Tensor):
        if not self.enabled:
            return
    ...
    def add_completed_tasks(self, step, reached: torch.Tensor):
        if not self.enabled:
            return
    ...
    def add_new_tasks(self, step, reached: torch.Tensor, target_locs: torch.Tensor):
        if not self.enabled:
            return
