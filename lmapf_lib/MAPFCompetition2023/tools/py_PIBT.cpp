...
class PIBTSolver {
public:
    PIBTSolver(size_t random_seed): rng(random_seed) {}
...
PYBIND11_MODULE(py_PIBT, m) {
	// optional module docstring
    // m.doc() = ;
    pybind11::class_<PIBTSolver>(m, "PIBTSolver")
        .def(pybind11::init<uint>())
        .def("solve", &PIBTSolver::solve)
        .def("playground", &PIBTSolver::playground);
}