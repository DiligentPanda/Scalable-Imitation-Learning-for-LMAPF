...
def get_atten_masks(curr_positions, max_atten_dist):
    '''
    curr_positions: B, A, 2
    '''
    
    # l-inf dist
    dists,_=torch.abs(curr_positions[:,:,None,:]-curr_positions[:,None,:,:]).max(dim=-1)
    atten_masks=dists<=max_atten_dist
    return atten_masks

def get_edge_feats(curr_positions, FoV_h, FoV_w):
    '''
    curr_positions: B, A, 2
    '''
    h=FoV_h//2
...
class Critic(nn.Module):
    def __init__(self, model_config, observation_space, action_space, custom_config, initialization):
        ...
        self.use_edge_feats=model_config["use_edge_feats"]
    ...
    def forward(self, **kwargs):
        ...
        if self.use_edge_feats:
            # B*A
            edge_feats = get_edge_feats(curr_positions.reshape(B,A,2), self.FOV_height, self.FOV_width)
        else:
            edge_feats = None
        
        atten_masks = get_atten_masks(curr_positions.reshape(B,A,2), self.max_atten_dist)
        h = h.reshape(B,A,-1)
        for i in range(self.num_encode_blocks):
            h = self.comm[i](h, atten_masks)
        h = h.reshape(B*A,-1)
...
class Actor(nn.Module):
    def __init__(
        ...
        self.comm=nn.ModuleList()
        for i in range(self.num_encode_blocks):
            encode_block = EncodeBlock(n_embd=self.hidden_dim,n_head=8,use_edge_feats=self.use_edge_feats)
            self.comm.append(encode_block)
    ...
    def forward(self, **kwargs):
        ...
        h = h.reshape(B,A,-1)
        for i in range(self.num_encode_blocks):
            h = self.comm[i](h, atten_masks, edge_feats)
        h = h.reshape(B*A,-1)
