...
class Backbone(nn.Module):
    ...
    def forward(
        ...
        priorities=original_observations[...,-5]
        curr_positions=original_observations[...,-4:-2]
        # target_positions=observations[...,-2:]
        ...
        # B,A
        priorities=priorities.view(B,A)
        # B,A,A
        priorities_comp=(priorities[:,:,None]>priorities[:,None,:]).float()
        # B,A,A,1
        priorities_comp=priorities_comp.unsqueeze(-1)
        
        # B,A,2
        curr_positions=curr_positions.view(B,A,2)
        # B,A,A,2
        relative_positions=curr_positions[:,:,None,2]-curr_positions[:,None,:,2]
        relative_positions=relative_positions/((self.FOV_height+self.FOV_width)*0.5)
        
        edge_feats=torch.cat([priorities_comp,relative_positions],dim=-1)
        ...
        # observations=observations.reshape(batch_size*self.num_agents,-1)
        edge_feats=edge_feats.reshape(batch_size*self.num_agents,-1)
        
        return {"obs_rep": obs_rep, "obs": original_observations, "edge_feats": edge_feats}
