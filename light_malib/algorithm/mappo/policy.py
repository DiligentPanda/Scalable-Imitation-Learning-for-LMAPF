...
def load_module(target: nn.Module, source: nn.Module, strict: bool=True):
...
def shape_adjusting(wrapped, instance, args, kwargs):
    ...
    TODO(rivers): This is really a bad design of the legacy code, how could we remove it?
    """
    ...
    #         Logger.error("{}: {}".format(k,v.shape))

    if EpisodeKey.EPISODE in rets:
        episode=rets.pop(EpisodeKey.EPISODE)
    else:
        episode=None
    ...
    if episode is not None:
        recover_rets[EpisodeKey.EPISODE]=episode
...
class MAPPO(Policy):
    ...
    def get_initial_state(self, batch_size) -> List[DataTransferType]:
        ...
        return {
            EpisodeKey.ACTOR_RNN_STATE: torch.zeros(
                (batch_size, self.actor.rnn_layer_num, self.actor.rnn_state_size), device=self.device, dtype=torch.float32
            ),
            EpisodeKey.CRITIC_RNN_STATE: torch.zeros(
                (batch_size, self.critic.rnn_layer_num, self.critic.rnn_state_size), device=self.device, dtype=torch.float32
            ),
        }

    def to_device(self, device, in_place=False):
        if in_place:
            self_copy=self
        else:
    ...
    def compute_action(self, **kwargs):
        ...
        with torch.set_grad_enabled(not inference):
            global_observations=kwargs[EpisodeKey.CUR_GLOBAL_OBS]
            ...
            if EpisodeKey.CUR_GLOBAL_STATE not in kwargs:
                global_states = global_observations
            else:
                global_states = kwargs[EpisodeKey.CUR_GLOBAL_STATE]
            
            if self.share_backbone:
                observations=self.backbone(
                    **{
                        EpisodeKey.CUR_GLOBAL_OBS: global_observations,
                        EpisodeKey.CUR_STATE: states,
                        EpisodeKey.CUR_OBS: observations,
                        EpisodeKey.CRITIC_RNN_STATE: critic_rnn_states,
                        EpisodeKey.DONE: rnn_masks
                    }
                )
                # actor and critic both use observations generated by the backbone
                states=observations

            actions, actor_rnn_states, action_log_probs, dist_entropy, logits = self.actor(
                **{
                    EpisodeKey.CUR_GLOBAL_OBS: global_observations,
                    EpisodeKey.CUR_OBS: observations,
            ...
            ret = {
                ...
                EpisodeKey.ACTOR_RNN_STATE: actor_rnn_states,
                EpisodeKey.CRITIC_RNN_STATE: critic_rnn_states,
                EpisodeKey.ACTION_LOGITS: logits
            }
            ...
            if not no_critic:    
                values, critic_rnn_states = self.critic(
                    **{
                        EpisodeKey.CUR_GLOBAL_STATE: global_states,
                        EpisodeKey.CUR_STATE: states,
                        EpisodeKey.CRITIC_RNN_STATE: critic_rnn_states,
                        EpisodeKey.DONE: rnn_masks
                    }
    ...
    def value_function(self, **kwargs):        
        ...
        with torch.set_grad_enabled(not inference):
            ...
            if self.share_backbone:
                observations=self.backbone(states,observations,critic_rnn_states,rnn_masks)
                # actor and critic both use observations generated by the backbone
                states=observations
            value, _ = self.critic(
                **{
    ...
    def load(dump_dir, **kwargs):
        ...
        if os.path.exists(actor_path):
            ...
            load_module(res.actor, actor)
            
        critic_path = os.path.join(dump_dir, "critic.pt")
        if os.path.exists(critic_path):
            critic = torch.load(critic_path, res.device)
            load_module(res.critic, critic)
        
        if res.share_backbone:
            backbone_path = os.path.join(dump_dir, "backbone.pt")
            if os.path.exists(backbone_path):
                backbone = torch.load(backbone_path, res.device)
                load_module(res.backbone, backbone)
                
        return res

    def load_soft(self, dump_dir):
        actor_path = os.path.join(dump_dir, "actor.pt")
        if os.path.exists(actor_path):
            actor = torch.load(actor_path, self.device)
            load_module(self.actor, actor,False)
            
        critic_path = os.path.join(dump_dir, "critic.pt")
        if os.path.exists(critic_path):
            critic = torch.load(critic_path, self.device)
            load_module(self.critic, critic,False)
        
        if self.share_backbone:
            backbone_path = os.path.join(dump_dir, "backbone.pt")
            if os.path.exists(backbone_path):
                backbone = torch.load(backbone_path, self.device)
                load_module(self.backbone, backbone,False)
