...
def discrete_autoregreesive_act(decoder, obs_rep, obs, edge_feats, act_neighboring_masks, batch_size, n_agent, action_dim,
    ...
    logits = decoder(shifted_action, obs_rep, obs, edge_feats, act_neighboring_masks, start_pos=-1)
    
    if available_actions is not None:
        logits[available_actions == 0] = -1e10  

    distri = Categorical(logits=logits)
    actions = distri.probs.argmax(dim=-1) if deterministic else distri.sample()
    
    action_logs = distri.log_prob(actions)
            
    return actions, action_logs, logits


def discrete_parallel_act(decoder, obs_rep, obs, edge_feats, act_neighboring_masks, action, batch_size, n_agent, action_dim,
    ...
    logit = decoder(shifted_action, obs_rep, obs, edge_feats, act_neighboring_masks, start_pos=-1)
    if available_actions is not None:
