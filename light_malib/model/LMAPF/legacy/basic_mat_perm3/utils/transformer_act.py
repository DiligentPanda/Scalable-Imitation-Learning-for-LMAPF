...
def discrete_autoregreesive_act(decoder, obs_rep, obs, act_neighboring_masks, batch_size, n_agent, action_dim,
    ...
    obs_rep_mean = torch.mean(obs_rep, dim=1, keepdim=True)
    shifted_obs_rep = torch.concat([obs_rep_mean,obs_rep],dim=1)[:,:-1,:]
    
    act_neighboring_masks = F.pad(act_neighboring_masks, (1,0), mode='constant', value=1)[:,:,:-1]

    for i in range(n_agent):
        logit = decoder(shifted_action[:, i:i+1], shifted_obs_rep[:,i:i+1], obs_rep[:, i:i+1], obs[:, i:i+1], act_neighboring_masks[:, i:i+1, :i+1], start_pos=i)
...
def discrete_parallel_act(decoder, obs_rep, obs, act_neighboring_masks, action, batch_size, n_agent, action_dim,
    ...
    logit = decoder(shifted_action, shifted_obs_rep, obs_rep, obs, act_neighboring_masks, start_pos=-1)
    if available_actions is not None:
