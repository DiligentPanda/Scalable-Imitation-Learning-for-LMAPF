...
from .mean_field import MeanFieldLayer
...
class Critic(nn.Module):
    ...
    def forward(self, **kwargs):
        ...
        R = observations.shape[0]//B
        
        assert len(observations.shape)==2
    
        observations=observations[...,:-5-self.max_conflict_pairs*5]
...
class Actor(nn.Module):
    def __init__(
        ...
        self.max_conflict_pairs=29
        self.mean_field=MeanFieldLayer(iterations=10)
    ...
    def forward(self, **kwargs):
        ...
        index_offsets=torch.arange(B,dtype=torch.long,device=observations.device)*R
        index_offsets=index_offsets.reshape(-1,1).repeat(1,R).reshape(-1)
        conflict_pairs=observations[...,-5-self.max_conflict_pairs*5:-5]
        conflict_pairs[...,0]+=index_offsets
        conflict_pairs[...,2]+=index_offsets
        observations=observations[...,:-5-self.max_conflict_pairs*5]
        ...
        logits=self.mean_field(logits, illegal_action_mask, conflict_pairs)
