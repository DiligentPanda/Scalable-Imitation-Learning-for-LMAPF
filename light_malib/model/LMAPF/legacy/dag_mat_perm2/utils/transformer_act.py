...
def discrete_autoregreesive_act(decoder, obs_rep, obs, act_neighboring_masks, batch_size, n_agent, action_dim,
    ...
    one_hot_action = torch.zeros((batch_size, n_agent, action_dim), device=obs.device, dtype=obs.dtype)
    ...
    # rivers The cache implementation here still has a bug
    # we need to carefully deal with the action
    for i in range(n_agent):
        logit = decoder(one_hot_action[:, i:i+1], obs_rep[:, i:i+1], obs[:, i:i+1], act_neighboring_masks[:, i:i+1, :i+1], start_pos=i)
        ...
        try:
            ...
            Logger.error("agent {}, logits: {}".format(i, logit))
            import traceback
        ...
        one_hot_action[:, i, :] = F.one_hot(action, num_classes=action_dim).float()
        
        if i!=n_agent-1:
            decoder.update_cache(one_hot_action[:, i:i+1], obs_rep[:, i:i+1], obs[:, i:i+1], act_neighboring_masks[:, i:i+1, :i+1], start_pos=i)
...
def discrete_parallel_act(decoder, obs_rep, obs, act_neighboring_masks, action, batch_size, n_agent, action_dim,
    ...
    action=action.long()
    one_hot_action = F.one_hot(action.squeeze(-1), num_classes=action_dim).float()  # (batch, n_agent, action_dim)
    # shifted_action = torch.zeros((batch_size, n_agent, action_dim), device=obs.device, dtype=obs.dtype)
    # shifted_action[:, :, :] = one_hot_action
    # start_pos = -1 means we don't use cache
    logit = decoder(one_hot_action, obs_rep, obs, act_neighboring_masks, start_pos=-1)
    if available_actions is not None:
