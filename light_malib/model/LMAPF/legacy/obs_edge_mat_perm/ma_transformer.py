...
class SelfAttention(nn.Module):
    ...
    def __init__(self, n_embd, n_head, n_agent, masked=False):
        ...
        self.attn_edge = nn.Sequential(init_(nn.Linear(3, n_embd), activate=True), nn.GELU(), nn.LayerNorm(n_embd),
                                      init_(nn.Linear(n_embd, self.n_head)))
    ...
    def forward(self, key, value, query, atten_masks, edge_feats, start_pos=-1):
        B, L, D = query.size()
        ...
        # B, A, A, nh
        edge_att = self.attn_edge(edge_feats)
        edge_att = edge_att.permute(0,3,1,2)

        # causal attention: (B, nh, L0, hs) x (B, nh, hs, L1) -> (B, nh, L0, L1)
        att = (q @ k.transpose(-2, -1) + edge_att) * (1.0 / math.sqrt(k.size(-1)))
...
class EncodeBlock(nn.Module):
    ...
    def forward(self, x, atten_masks, edge_feats):
...
class DecodeBlock(nn.Module):
    ...
    def forward(self, x, rep_enc, atten_masks, edge_feats, start_pos=-1):
        # TODO(rivers): we should think about this design more
        x = self.ln1(x + self.attn1(x, x, x, atten_masks, edge_feats, start_pos))
        # TODO(rivers): why not merge x and rep_enc here?
        x = self.ln2(rep_enc + self.attn2(key=x, value=x, query=rep_enc, atten_masks=atten_masks, edge_feats=edge_feats, start_pos=start_pos))
...
class Encoder(nn.Module):
    ...
    def forward(self, state, obs, atten_masks, edge_feats):
        ...
        for block in self.blocks:
            rep = block(rep, atten_masks, edge_feats)
...
class Decoder(nn.Module):
    ...
    def forward(self, action, obs_rep, obs, edge_feats, atten_masks, start_pos=-1):
        # action: (batch, n_agent, action_dim), one-hot/logits?
        # obs_rep: (batch, n_agent, n_embd)
        for block in self.blocks:
            x = block(obs_rep, atten_masks, edge_feats)
        logit = self.head(x)
