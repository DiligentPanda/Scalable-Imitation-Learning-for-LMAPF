...
class Critic(nn.Module):
    def __init__(self, model_config, observation_space, action_space, custom_config, initialization):
        ...
        self.rnn_state_size=self.hidden_dim
    ...
    def forward(self, **kwargs):
        ...
        critic_rnn_states= critic_rnn_states.squeeze(1)
        critic_rnn_states = self.gru(h,critic_rnn_states)
        # B*A,1
        values = self.out(critic_rnn_states+h)
        
        values = values[reverse_perm_indices]
        
        critic_rnn_states=critic_rnn_states.unsqueeze(1)
...
class Actor(nn.Module):
    ...
    def forward(self, **kwargs):
        ...
        # Logger.error("rnn shape: {}".format(actor_rnn_states.shape))
        actor_rnn_states=actor_rnn_states.squeeze(1)
        actor_rnn_states = self.gru(h,actor_rnn_states)
        
        # B*A,5
        logits=self.out(actor_rnn_states+h)
        ...
        actor_rnn_states=actor_rnn_states.unsqueeze(1)
