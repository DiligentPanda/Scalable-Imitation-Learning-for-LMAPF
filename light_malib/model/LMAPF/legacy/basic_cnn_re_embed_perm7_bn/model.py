...
class ReEmbedBlock(nn.Module):
    def __init__(self, obs_dim, action_dim, hidden_dim):
        ...
        self.re_embed=ReEmbed(obs_dim, action_dim, self.hidden_dim)
    ...
    def forward(self, global_feats, curr_positions, local_feats, observations, action_masks):
        # B*A,5
        logits=self.out(local_feats.detach())
        illegal_action_mask = 1-action_masks
        logits=logits-1e10*illegal_action_mask
        probs=torch.softmax(logits,dim=-1)
        
        local_feats = torch.cat([local_feats,probs.detach()],dim=-1)
        # # for simplicity, we can stop gradient flowing back
        # local_feats = local_feats.detach()
        
        ctx_feats = self.re_embed(global_feats, curr_positions, local_feats, observations)
        local_feats = ctx_feats + self.proj(local_feats)
        local_feats = self.ln(local_feats) 
        return local_feats, logits
...
class Actor(nn.Module):
    def __init__(
        ...
        for i in range(self.num_re_embed_blocks):
            re_embed_block=ReEmbedBlock(self.in_dim, self.action_dim, self.hidden_dim)
    ...
    def forward(self, **kwargs):
        ...
        B, _, H, W = global_observations.shape
        A=observations.shape[0]//B
        ...
        if self.num_re_embed_blocks!=0:
            ...
            for i in range(self.num_re_embed_blocks):
                h, logits = self.comm[i](gh, curr_positions, h, observations, action_masks)
