...
class Actor(nn.Module):
    ...
    def forward(self, **kwargs):
        ...
        # B*A, -1
        h_ctx = self.re_embed1(gh, curr_positions, h)
        h = h_ctx + self.proj1(h)
        h = self.ln1(h) 
        
        h_ctx = self.re_embed2(gh, curr_positions, h)
        h = h_ctx + self.proj2(h)
        h = self.ln2(h)        
