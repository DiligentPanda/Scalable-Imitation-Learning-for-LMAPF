...
class ReEmbedBlock(nn.Module):
    def __init__(self, obs_dim, hidden_dim):
        ...
        self.re_embed=ReEmbed(obs_dim, self.hidden_dim)
    ...
    def forward(self, global_feats, curr_positions, local_feats, observations):
...
class Actor(nn.Module):
    def __init__(
        ...
        for i in range(self.num_re_embed_blocks):
            re_embed_block=ReEmbedBlock(self.in_dim, self.hidden_dim)
    ...
    def forward(self, **kwargs):
        ...
        if self.num_re_embed_blocks!=0:
            gh = torch.zeros(size=(B,self.hidden_dim,H,W), dtype=observations.dtype, device=observations.device)
            
            for i in range(self.num_re_embed_blocks):
                h = self.comm[i](gh, curr_positions, h, observations)
