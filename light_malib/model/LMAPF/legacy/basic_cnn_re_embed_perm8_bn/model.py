...
class Actor(nn.Module):
    def __init__(
        ...
        self.temporal = nn.Sequential(
            nn.Conv1d(self.hidden_dim, self.hidden_dim, kernel_size=self.temporal_dim),
        )
    ...
    def forward(self, **kwargs):
        ...
        h = h.reshape(B//L,L,A,self.hidden_dim)
        h = h.transpose(0,2,3,1)
        h = h.reshape(-1,self.hidden_dim,L)
        
        ...
        h = self.temporal(h)
        
        h = h.reshape(B//L,A,self.hidden_dim,L)
