...
from light_malib.framework.ppo_runner import PPORunner
import time
import os
import yaml
...
def start_cluster():
    ...
    _system_config={
        "object_spilling_config": json.dumps(
            {"type": "filesystem", "params": {"directory_path": "/root/autodl-tmp/spill"}},
        )
    }
    
    try:
        cluster_start_info = ray.init(address="auto",_system_config=_system_config)
    except ConnectionError:
        Logger.warning("No active cluster detected, will create local ray instance.")
        cluster_start_info = ray.init(resources={},_system_config=_system_config)
...
def main():
    ...
    if cfg.framework.sync_training and cfg.framework.get('on_policy', True):
        ...
        if int(cfg.rollout_manager.worker.sample_length)==0:
            sample_length=cfg.rollout_manager.worker.rollout_length
        else:
            sample_length=cfg.rollout_manager.worker.sample_length
        assert cfg.training_manager.batch_size==cfg.rollout_manager.batch_size*(cfg.rollout_manager.worker.rollout_length//sample_length)
