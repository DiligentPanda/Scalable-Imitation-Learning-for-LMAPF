...
class Critic(nn.Module):
    def __init__(self, model_config, observation_space, action_space, custom_config, initialization):
        ...
        self.out=nn.Linear(self.global_hidden_dim+1,1)
        ...
        self.global_backbone=nn.Sequential(
            ...
            nn.ReLU(),
            nn.MaxPool2d(2,2),
            nn.LayerNorm([self.global_hidden_dim,self.map_height//2,self.map_width//2]),
            nn.Conv2d(self.global_hidden_dim,self.global_hidden_dim,kernel_size=3,padding=1),
            nn.ReLU(),
            nn.MaxPool2d(2,2),
            nn.LayerNorm([self.global_hidden_dim,self.map_height//4,self.map_width//4]),
            nn.Conv2d(self.global_hidden_dim,self.global_hidden_dim,kernel_size=3,padding=1),
            nn.ReLU(),
            nn.MaxPool2d(2,2),
            nn.LayerNorm([self.global_hidden_dim,self.map_height//8,self.map_width//8]),
            nn.Conv2d(self.global_hidden_dim,self.global_hidden_dim,kernel_size=3,padding=1),
            nn.ReLU(),
            nn.MaxPool2d(2,2),
            nn.LayerNorm([self.global_hidden_dim,self.map_height//16,self.map_width//16]),
            nn.Conv2d(self.global_hidden_dim,self.global_hidden_dim,kernel_size=3,padding=1),
            nn.ReLU(),
            nn.MaxPool2d(2,2),
            nn.LayerNorm([self.global_hidden_dim,self.map_height//32,self.map_width//32]),
        )   
    ...
    def forward(self, **kwargs):
        ...
        steps=observations[...,-6:-5]

        global_observations=self.global_backbone(global_observations)
        global_observations=global_observations.reshape(B,1,-1).repeat(1,A,1).reshape(B*A,-1)
        ...
        values = self.out(torch.cat([global_observations,steps],dim=1))
...
class Actor(nn.Module):
    def __init__(
        ...
        self.out=nn.Linear(self.hidden_dim+self.global_hidden_dim,5)
    ...
    def forward(self, **kwargs):
        ...
        h = h.reshape(h.shape[0],-1)
        # B*A,5
        logits = self.out(torch.cat([h,global_observations],dim=1))
