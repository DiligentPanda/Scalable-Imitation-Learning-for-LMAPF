...
def simple_data_generator(data, num_mini_batch, device, shuffle=True):
    ...
    assert shuffle==False,"no need to do shuffle in simple_data_generator now"
...
def simple_team_data_generator(data, num_mini_batch, device, shuffle=False):
    ...
    assert num_mini_batch<=len_traj and len_traj%num_mini_batch==0, "TODO(rivers): experiment code"
    ...
    if num_mini_batch==1:
        ...
        for k in data:
            try:
                ...
                if isinstance(data[k], np.ndarray):
                    ...
                    batch[k] = data[k].to(device)
                global_timer.time("data_copy_start", "data_copy_end", "data_copy")
                batch[k] = batch[k].reshape(len_traj*n_rollout_threads, *data[k].shape[2:])
            except Exception:
                Logger.error("k: {}".format(k))
                raise Exception
        yield batch
        ...
        mini_batch_size = n_rollout_threads // num_mini_batch
        
        for i in range(mini_batch_size):
            sidx=i*mini_batch_size
            eidx=(i+1)*mini_batch_size
            mini_batch = {}
            for k in data:
                try:
                    global_timer.record("data_copy_start")
                    if isinstance(data[k], np.ndarray):
                        mini_batch[k] = torch.from_numpy(data[k][:,sidx:eidx]).to(device)
                    else:
                        mini_batch[k] = data[k][:,sidx:eidx].to(device)
                    global_timer.time("data_copy_start", "data_copy_end", "data_copy")
                    mini_batch[k] = mini_batch[k].reshape(len_traj*mini_batch_size, *data[k].shape[2:])
                except Exception:
                    Logger.error("k: {}".format(k))
                    raise Exception
            yield mini_batch
