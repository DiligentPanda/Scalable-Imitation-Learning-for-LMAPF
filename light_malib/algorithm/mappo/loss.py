...
class MAPPOLoss(LossFunc):
    ...
    def reset(self, policy, config):
        ...
        assert self.sub_algorithm_name in ["BC","IPPO","MAPPO","CoPPO","HAPPO","A2PO","Dagger"]
        ...
        if self.sub_algorithm_name in ["IPPO","MAPPO"]:
            ...
            self._use_cum_sequence=True
            self._agent_seq=[]
        elif self.sub_algorithm_name in ["BC","Dagger"]:
            self._use_seq=False
        else:
    ...
    def setup_optimizers(self, *args, **kwargs):
        ...
        if len(list(self._policy.critic.parameters()))>0:
            ...
            self.param_names.append("critic")
        ...
        if self._policy.share_backbone and len(list(self._policy.backbone.parameters()))>0:
            ...
            self.param_names.append("backbone")
    ...
    def loss_compute(self, sample, step_optimizer:bool=True, grad_accum_steps:int=1):
        self.n_opt_steps+=1
        ...
        if self._use_seq:
            ...
            return self.loss_compute_simultaneous(sample, step_optimizer=step_optimizer, grad_accum_steps=grad_accum_steps)
    ...
    def loss_compute_simultaneous(
        ...
        if self.sub_algorithm_name in ["BC","Dagger"]:
            (
                ...
                sample[EpisodeKey.CUR_GLOBAL_STATE],
                sample[EpisodeKey.CUR_GLOBAL_OBS],
            ...
            ret = self._policy.compute_action(
                **{
                    EpisodeKey.CUR_STATE: state_batch,
                    EpisodeKey.CUR_OBS: obs_batch,
                    EpisodeKey.CUR_GLOBAL_STATE: global_state_batch,
                    EpisodeKey.CUR_GLOBAL_OBS: global_obs_batch,
            ...
            action_logits=ret[EpisodeKey.ACTION_LOGITS]
            dist_entropy=ret[EpisodeKey.ACTION_ENTROPY]    
            
            assert len(action_logits.shape) in [2, 3] and len(guiding_actions_batch.shape)==1, "{} vs {}".format(action_logits.shape,guiding_actions_batch.shape)
            if len(action_logits.shape)==3:
                guiding_actions_batch = guiding_actions_batch.unsqueeze(-1).repeat(1,action_logits.shape[-1])
            
            policy_loss=F.cross_entropy(action_logits,guiding_actions_batch)  
            
            predictions=torch.argmax(action_logits,dim=-1)
            correctness = (torch.argmax(action_logits,dim=-1) == guiding_actions_batch).float().mean()
            
            # predictions_py=predictions.detach().cpu().numpy()
            # guiding_actions_py=guiding_actions_batch.detach().cpu().numpy()
            # confusion_matrix=metrics.confusion_matrix(guiding_actions_py, predictions_py, normalize="true")
            # Logger.error("confusion matrix:\n {}".format(confusion_matrix))
            ...
            if step_optimizer:     
                if self._use_max_grad_norm:
                    for name, param_group in zip(self.param_names,self.optimizer.param_groups):
                        grad_norm=torch.nn.utils.clip_grad_norm_(
                            param_group["params"], self.max_grad_norm
                        )
                        param_grad_norms[name+"_grad_norm"]=float(grad_norm.cpu().numpy())
            ...
            stats = dict(
                ...
                correctness=float(correctness.detach().mean().cpu().numpy())
            )        
            stats.update(param_grad_norms)
        ...
        #     guiding_actions_batch = actions_batch
        #     actions_batch = init_actions_batch
        # else:
        #     guiding_actions_batch = None
        ...
        if update_actor:
            ...
            if guiding_actions_batch is not None:
                action_logits=ret[EpisodeKey.ACTION_LOGITS]
                guiding_loss=F.cross_entropy(action_logits,guiding_actions_batch)
            else:
                guiding_loss=0.0

            policy_loss = policy_action_loss * self._policy.custom_config["policy_loss_coef"] + policy_entropy_loss * self._policy.custom_config["entropy_coef"] + guiding_loss * self._policy.custom_config["guiding_loss_coef"]
        ...
        if update_actor:
            ...
            stats = dict(
                ...
                policy_loss=float(policy_loss.detach().cpu().numpy()),
                policy_action_loss=float(policy_action_loss.detach().cpu().numpy()),
                guiding_loss=float(guiding_loss.detach().cpu().numpy()) if isinstance(guiding_loss,torch.Tensor) else guiding_loss,
                value_loss=float(value_loss.detach().cpu().numpy()) if isinstance(value_loss,torch.Tensor) else value_loss,
                entropy=float(dist_entropy.detach().mean().cpu().numpy()),
                approx_kl=approx_kl,
            )
            stats.update(param_grad_norms)
