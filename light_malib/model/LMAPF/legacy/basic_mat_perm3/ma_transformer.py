...
class Decoder(nn.Module):
    ...
    def __init__(self, obs_dim, action_dim, n_block, n_embd, n_head, n_agent,
        ...
        if self.dec_actor:
            ...
            self.shifted_obs_encoder = nn.Sequential(init_(nn.Linear(n_embd, n_embd, bias=False), activate=True),
                                                    nn.GELU())
    ...
    def forward(self, action, shifted_obs, obs_rep, obs, atten_masks, start_pos=-1):
        ...
        if self.dec_actor:
            ...
            shifted_obs_embeddings = self.shifted_obs_encoder(shifted_obs)
            x = self.ln(action_embeddings+shifted_obs_embeddings)
            for block in self.blocks:
