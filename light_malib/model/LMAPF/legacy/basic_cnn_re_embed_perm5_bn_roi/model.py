...
from .roi_sampling import OrderedRoISampling
...
class Critic(nn.Module):
    def __init__(self, model_config, observation_space, action_space, custom_config, initialization):
        ...
        self.roi_h = 3
        self.roi_w = 3
        self.roi_sampling = OrderedRoISampling(self.roi_h,self.roi_w)
    ...
    def forward(self, **kwargs):
        ...
        agent_batch_indices=torch.arange(B,dtype=torch.long,device=observations.device)*A
        agent_batch_indices=agent_batch_indices.reshape(B,1).repeat(1,A).reshape(-1)
        
        batch_indices=torch.arange(B,dtype=torch.long,device=observations.device)
        ...
        # B*A, C, h, w        
        rois = self.roi_sampling(global_observations, batch_indices, curr_positions, target_positions)
        
        # B*A, C, 1 , 1
        rois = self.roi_backbone(rois)
        # B*A, C
...
class Actor(nn.Module):
    ...
    def forward(self, **kwargs):
        ...
        # perm_indices+=batch_indices
        reverse_perm_indices+=agent_batch_indices
        ...
        for i in range(self.num_re_embed_blocks):
            h = self.comm[i](global_observations, curr_positions, h)
